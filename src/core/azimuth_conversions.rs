...
pub fn convert_theta_vec(theta: ArrayView1<f64>) -> Array1<f64> {
    let theta_new: Array1<f64> = theta
        .iter()
        .map(|&theta_i| match theta_i {
            theta_i if theta_i > 180.0 => theta_i - 360.0,
            theta_i if theta_i < -180.0 => theta_i + 360.0,
            _ => theta_i,
        })
        .collect();
...
pub fn convert_branch_360(theta: f64) -> f64 {
    match theta {
        ...
        //theta if theta >= 0.0 => 360.0 - theta,
        _ => theta % 360.0,
    }
}
...
mod test_azimuth_conversions {
    ...
    fn test_convert_theta_vec() {
        ...
        let theta_new = convert_theta_vec(theta.view());
